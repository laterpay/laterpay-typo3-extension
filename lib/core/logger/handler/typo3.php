...
class tx_laterpay_core_logger_handler_typo3 extends tx_laterpay_core_logger_handler_abstract {
	...
	protected function getTabs() {
		...
		return array(
				array(
						'name'		=> tx_laterpay_helper_string::tr( 'Requests', 'laterpay' ),
						'content'	=> array_merge(t3lib_div::_GET(), t3lib_div::_POST()),
				),
				array(
						'name'		=> tx_laterpay_helper_string::tr( 'Session', 'laterpay' ),
						'content'	=> isset( $_SESSION ) ? $_SESSION : array(),
				),
				array(
						'name'		=> sprintf( tx_laterpay_helper_string::tr( 'Cookies<span class="lp_debugger-tabs__count">%s</span>', 'laterpay' ), count( $_COOKIE ) ),
						'content'	=> $_COOKIE,
				),
				array(
						'name'		=> tx_laterpay_helper_string::tr( 'System Config', 'laterpay' ),
						'content'	=> $this->getSystemInfo(),
				),
				array(
						'name'		=> tx_laterpay_helper_string::tr( 'Plugin Config', 'laterpay' ),
...
// 				'Multisite'                 => is_multisite() ? tx_laterpay_helper_string::tr( 'yes', 'laterpay' ) : tx_laterpay_helper_string::tr( 'no', 'laterpay' ),
// 				'WordPress memory limit'    => ( $this->let_to_num( WP_MEMORY_LIMIT ) / 1024 ) . ' MB',
// 				'Active plugins'            => implode( ', ', $plugins ),
// 				'Network active plugins'    => is_multisite() ? $network_plugins : tx_laterpay_helper_string::tr( 'none', 'laterpay' ),
...
// 				'Active theme'              => $theme,
	...
	public function flushRecords() {
		...
		foreach ($tabsNamesAndContents  as $key => $tab ) {
			...
			$tabs .= sprintf(self::DEBUG_TABS_TEMPLATE, tx_laterpay_helper_string::tr( $tab['name'], 'laterpay' ));
		...
		$out = sprintf(
			...
			sprintf( tx_laterpay_helper_string::tr( '%s Memory Usage', 'laterpay' ), number_format( memory_get_peak_usage() / pow( 1024, 2 ), 1 ) . ' MB' ),
			tx_laterpay_helper_string::tr( 'Debugger', 'laterpay' ),
			tx_laterpay_helper_string::tr( 'Messages', 'laterpay' ),
			count( $this->records ),
			$tabs,
