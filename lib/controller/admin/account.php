...
class tx_laterpay_controller_admin_account extends tx_laterpay_controller_abstract {
	...
	public function loadAssets() {
		...
		$this->localizeScript('lpVars',
			array(
				'ajaxUtl' 				=> 'ajax.php',
				'i18nApiKeyInvalid'		=> tx_laterpay_helper_string::tr('The API key you entered is not a valid LaterPay API key!', 'laterpay'),
				'i18nMerchantIdInvalid'	=> tx_laterpay_helper_string::tr('The Merchant ID you entered is not a valid LaterPay Merchant ID!', 'laterpay'),
				'i18nPreventUnload'		=> tx_laterpay_helper_string::tr('LaterPay does not work properly with invalid API credentials.', 'laterpay')
			));
	...
	protected static function updateMerchantId($isLive = NULL) {
		...
		if ($merchantIdForm->isValid()) {
			...
			$fResult = array(
				'success' => TRUE,
				'message' => sprintf(tx_laterpay_helper_string::tr('%s Merchant ID verified and saved.', 'laterpay'), ucfirst($merchantIdType))
			);
		} elseif (strlen($merchantId) == 0) {
			tx_laterpay_config::updateOption(sprintf('laterpay_%s_merchant_id', $merchantIdType), '');
			$fResult = array(
				'success' => TRUE,
				'message' => sprintf(tx_laterpay_helper_string::tr('The %s Merchant ID has been removed.', 'laterpay'), ucfirst($merchantIdType))
			);
		} else {
			$fResult = array(
				'success' => FALSE,
				'message' => sprintf(
					tx_laterpay_helper_string::tr('The Merchant ID you entered is not a valid LaterPay %s Merchant ID!', 'laterpay'),
	...
	protected static function updateApiKey($isLive = NULL) {
		...
		if ($apiKeyForm->isValid()) {
			...
			$fResult = array(
				'success' => TRUE,
				'message' => sprintf(tx_laterpay_helper_string::tr('Your %s API key is valid. You can now make %s transactions.', 'laterpay'),
			...
			$fResult = array(
				'success' => TRUE,
				'message' => sprintf(tx_laterpay_helper_string::tr('The %s API key has been removed.', 'laterpay'), ucfirst($apiKeyType))
			);
		} else {
			$fResult = array(
				'success' => FALSE,
				'message' => sprintf(tx_laterpay_helper_string::tr('The API key you entered is not a valid LaterPay %s API key!', 'laterpay'),
					ucfirst($apiKeyType))
			);
		}
	...
	protected static function updatePluginMode() {
		...
		if (! $pluginModeForm->isValid(t3lib_div::_POST())) {
			$fResult = array(
				'success' => FALSE,
				'message' => tx_laterpay_helper_string::tr('Error occurred. Incorrect data provided.', 'laterpay')
			);
		}
		...
		if ($result) {
			...
			$fResult = array(
				'success' 	=> FALSE,
				'mode' 		=> 'test',
				'message'	=> tx_laterpay_helper_string::tr('The LaterPay plugin needs valid API credentials to work.', 'laterpay')
			);
		}
	...
	public static function updatePluginVisibilityInTestMode() {
		...
		if (! $pluginTestModeForm->isValid(t3lib_div::_POST())) {
			$fResult = array(
				'success' 	=> FALSE,
				'mode' 		=> 'test',
				'message'	=> tx_laterpay_helper_string::tr('An error occurred. Incorrect data provided.', 'laterpay')
			);
		}
		...
		if ($hasInvalidCredentials) {
			...
			if ($isInVisibleTestMode) {
				$message = tx_laterpay_helper_string::tr('The plugin is in <strong>visible</strong> test mode now.', 'laterpay');
			} else {
				$message = tx_laterpay_helper_string::tr('The plugin is in <strong>invisible</strong> test mode now.', 'laterpay');
			}
