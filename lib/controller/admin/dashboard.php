...
class tx_laterpay_controller_admin_dashboard extends tx_laterpay_controller_abstract {
	...
	public function loadAssets() {
		...
		$i18n = array(
			'endingIn'		=> tx_laterpay_helper_string::trX('ending in', 'used in wp_localize_script for the flot graph in loadTimePassLifecycles()', 'laterpay'),
			'month'			=> tx_laterpay_helper_string::trX('month', 'used in wp_localize_script for the flot graph in loadTimePassLifecycles()', 'laterpay'),
			'months'		=> tx_laterpay_helper_string::trX('months', 'used in wp_localize_script for the flot graph in loadTimePassLifecycles()', 'laterpay'),
			'weeksLeft'		=> tx_laterpay_helper_string::trX('weeks left', 'used in wp_localize_script as x-axis label for loadTimePassLifecycles()', 'laterpay'),
			'noData'		=> tx_laterpay_helper_string::tr('No data available', 'laterpay'),
			'tooltips' 	=> array(
				'day' 		=> array(
					'next' => tx_laterpay_helper_string::tr('Show next day', 'laterpay'),
					'prev' => tx_laterpay_helper_string::tr('Show previous day', 'laterpay')
				),
				'week' 		=> array(
					'next' => tx_laterpay_helper_string::tr('Show next 8 days', 'laterpay'),
					'prev' => tx_laterpay_helper_string::tr('Show previous 8 days', 'laterpay')
				),
				'2-weeks' 	=> array(
					'next' => tx_laterpay_helper_string::tr('Show next 2 weeks', 'laterpay'),
					'prev' => tx_laterpay_helper_string::tr('Show previous 2 weeks', 'laterpay')
				),
				'month' 	=> array(
					'next' => tx_laterpay_helper_string::tr('Show next month', 'laterpay'),
					'prev' => tx_laterpay_helper_string::tr('Show previous month', 'laterpay')
				)
			)
		);
	...
	private function validateAjaxSectionCallback() {
		...
		if (! isset($post['section'])) {
			$fResult = array(
				'message'	=> tx_laterpay_helper_string::tr('Error, missing section on request', 'laterpay'),
				'step' 		=> 3,
			);
		} elseif (! array_key_exists($post['section'], $this->ajaxSections)) {
			$fResult = array(
				'message'	=> sprintf(tx_laterpay_helper_string::tr('Section is not allowed <code>%s</code>', 'laterpay'), $post['section']),
				'step' 		=> 4,
			);
		} elseif (! method_exists($this, $this->ajaxSections[$post['section']])) {
			$fResult = array(
				'message'	=> sprintf(tx_laterpay_helper_string::tr('Invalid section <code>%s</code>', 'laterpay'), $post['section']),
				'step' 		=> 4,
			);
		}
