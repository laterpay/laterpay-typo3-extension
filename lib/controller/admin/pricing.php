...
class tx_laterpay_controller_admin_pricing extends tx_laterpay_controller_abstract {
	...
	public function loadAssets() {
		...
		$i18n = array(
			// bulk price editor
			'make'						=> tx_laterpay_helper_string::tr('Make', 'laterpay'),
			'free'						=> tx_laterpay_helper_string::tr('free', 'laterpay'),
			'to'						=> tx_laterpay_helper_string::tr('to', 'laterpay'),
			'by'						=> tx_laterpay_helper_string::tr('by', 'laterpay'),
			'toGlobalDefaultPrice'		=> tx_laterpay_helper_string::tr('to global default price of', 'laterpay'),
			'toCategoryDefaultPrice'	=> tx_laterpay_helper_string::tr('to category default price of', 'laterpay'),
			'updatePrices'				=> tx_laterpay_helper_string::tr('Update Prices', 'laterpay'),
			'delete'					=> tx_laterpay_helper_string::tr('Delete', 'laterpay'),

			// time pass editor
			'confirmDeleteTimePass'		=> tx_laterpay_helper_string::tr('Every user, who owns this pass, will lose his access.', 'laterpay'),
			'voucherText'				=> tx_laterpay_helper_string::tr('allows purchasing this pass for', 'laterpay'),
			'timesRedeemed'				=> tx_laterpay_helper_string::tr('times redeemed.', 'laterpay')
		);
		...
		$this->localizeScript('lpVars',
			array(
				...
				'inCategoryLabel'		=> tx_laterpay_helper_string::tr('All posts in category', 'laterpay'),
	...
	public function renderPage() {
		...
		$bulkActions = array(
			'set'		=> tx_laterpay_helper_string::tr('Set price of', 'laterpay'),
			'increase'	=> tx_laterpay_helper_string::tr('Increase price of', 'laterpay'),
			'reduce'	=> tx_laterpay_helper_string::tr('Reduce price of', 'laterpay'),
			'free'		=> tx_laterpay_helper_string::tr('Make free', 'laterpay'),
			'reset'		=> tx_laterpay_helper_string::tr('Reset', 'laterpay')
		);

		$bulkSelectors = array(
			'all' => tx_laterpay_helper_string::tr('All posts', 'laterpay')
		);
	...
	protected function updateGlobalDefaultPrice() {
		...
		if (! $globalPriceForm->isValid($post)) {
			return array(
				...
				'message'						=> tx_laterpay_helper_string::tr('The price you tried to set is outside the allowed range of 0 or 0.05-149.99.', 'laterpay')
			);
		}
		...
		if ($globalPrice == 0) {
			$message = tx_laterpay_helper_string::tr('All posts are free by default now.', 'laterpay');
		} else {
			$message = sprintf(tx_laterpay_helper_string::tr('The global default price for all posts is %s %s now.', 'laterpay'), $localizedGlobalPrice,
				$currencyName);
		}
	...
	protected function changePostsPrice() {
		...
		if ($bulkPriceForm->isValid()) {
			...
			// flash message parts
			$messageParts 		= array(
				'all'			=> tx_laterpay_helper_string::tr('The prices of all posts', 'laterpay'),
				'category' 		=> '',
				'have_been'		=> tx_laterpay_helper_string::tr('have been', 'laterpay'),
				'action'		=> tx_laterpay_helper_string::tr('set', 'laterpay'),
				'preposition'	=> tx_laterpay_helper_string::tr('to', 'laterpay'),
				'amount' 		=> '',
				'unit' 			=> '',
			);
			...
			switch ($action) {
				case 'set':
					...
					$messageParts['action']			= tx_laterpay_helper_string::tr('set', 'laterpay');
					$messageParts['preposition']	= tx_laterpay_helper_string::tr('to', 'laterpay');
				...
				case 'reduce':
					...
					// set flash message parts
					$messageParts['action']			= $isReduction ? tx_laterpay_helper_string::tr('decreased', 'laterpay') : tx_laterpay_helper_string::tr('increased', 'laterpay');
					$messageParts['preposition']	= tx_laterpay_helper_string::tr('by', 'laterpay');
				...
				case 'free':
					...
					$messageParts['all']			= tx_laterpay_helper_string::tr('All posts', 'laterpay');
					$messageParts['action']			= tx_laterpay_helper_string::tr('made free', 'laterpay');
					$messageParts['preposition'] 	= '';
					break;

				case 'reset':
					$messageParts['action'] = tx_laterpay_helper_string::tr('reset', 'laterpay');
					if ($updateAll) {
						$newPrice = tx_laterpay_config::getOption('laterpay_global_price');

						// set flash message parts
						$messageParts['preposition']	= tx_laterpay_helper_string::tr('to global default price of', 'laterpay');
	...
	protected function saveBulkOperation() {
		...
		if ($saveBulkOperationForm->isValid()) {
			...
			return array(
				...
				'message' => tx_laterpay_helper_string::tr('Bulk operation saved.', 'laterpay')
			);
		}
	...
	protected function passFormSave() {
		...
		if ($savePassForm->isValid()) {
			...
			$fResult = array(
				...
				'message'	=> tx_laterpay_helper_string::tr('Pass saved.', 'laterpay')
			);
		} else {
			$fResult = array(
				'success' 	=> FALSE,
				'errors' 	=> $savePassForm->getErrors(),
				'message'	=> tx_laterpay_helper_string::tr('An error occurred when trying to save the pass. Please try again.', 'laterpay')
			);
		}
	...
	protected function passDelete() {
		...
		if (isset($postData)) {
			...
			$fResult = array(
				'success' => TRUE,
				'message' => tx_laterpay_helper_string::tr('Pass deleted.', 'laterpay')
			);
		} else {
			$fResult = array(
				'success' => FALSE,
				'message' => tx_laterpay_helper_string::tr('The selected pass was deleted already.', 'laterpay')
			);
		}
	...
	private function saveLandingPage() {
		...
		if ($landingPageForm->isValid()) {
			...
			return array(
				'success' => TRUE,
				'message' => tx_laterpay_helper_string::tr('Landing page saved.', 'laterpay')
			);
		} else {
			// show an error message, if the provided URL is not valid
			return array(
				'success' 	=> FALSE,
				'message'	=> tx_laterpay_helper_string::tr('The landing page you entered is not a valid URL.', 'laterpay'),
